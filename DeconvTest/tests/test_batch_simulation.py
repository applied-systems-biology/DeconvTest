...
class TestSimulation(unittest.TestCase):
    ...
    def test_simulate(self):
        ...
        sim.resize_batch('data/convolved/', 'data/resized/', resolutions=[4], print_progress=False)
        files = os.listdir('data/resized')
        self.assertEqual(len(files), 3)
        files = os.listdir('data/resized/psf_sigma_1.5_aspect_ratio_1.5_voxel_size_[4_4_4]')
        self.assertEqual(len(files), 4)
        ...
        files = os.listdir('data/noise/psf_sigma_1.5_aspect_ratio_1.5_'
                           'voxel_size_[4_4_4]_noise_gaussian_snr=10_poisson_snr=None')
        self.assertEqual(len(files), 4)
        shutil.rmtree('data/')
