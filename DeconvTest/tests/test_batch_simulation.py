...
class TestSimulation(unittest.TestCase):
    ...
    def test_generate_cells(self):
        ...
        sim.generate_cells_batch(params_file='data/params.csv', outputfolder='data/cells/',
                                 input_voxel_size=0.8, print_progress=False, max_threads=2)
    ...
    def test_simulate(self):
        ...
        sim.generate_cells_batch(params_file='data/params.csv', outputfolder='data/cells/',
                                 input_voxel_size=1, print_progress=False, max_threads=2)
        ...
        sim.resize_batch('data/convolved/', 'data/resized/', voxel_sizes_for_resizing=[4],
                         print_progress=False, max_threads=2)
        ...
        sim.add_noise_batch('data/resized/', 'data/noise/', noise_kind=['gaussian', 'poisson'],
                            snr=[None, 10], test_snr_combinations=True, print_progress=False, max_threads=2)
    ...
    def test_simulate_stack(self):
        sim.generate_cell_parameters('data/stack_params.csv', number_of_stacks=2)
        sim.generate_cells_batch(params_file='data/stack_params.csv', outputfolder='data/stacks',
                                 input_voxel_size=1, stack_size_microns=[10, 30, 30], print_progress=False, max_threads=2)
        sim.generate_psfs_batch('data/psfs', psf_sigmas=[1.5], psf_aspect_ratios=[1.5],
                                input_voxel_size=1, print_progress=False, max_threads=2)
        sim.convolve_batch('data/stacks/', 'data/psfs', 'data/convolved/', print_progress=False, max_threads=2)
        files = os.listdir('data/convolved')
        self.assertEqual(len(files), 3)
        files = os.listdir('data/convolved/psf_sigma_1.5_aspect_ratio_1.5')
        self.assertEqual(len(files), 4)

        sim.resize_batch('data/convolved/', 'data/resized/', voxel_sizes_for_resizing=[3],
                         print_progress=False, max_threads=2)
        files = os.listdir('data/resized')
        self.assertEqual(len(files), 3)
        ...
        sim.add_noise_batch('data/resized/', 'data/noise/', noise_kind='gaussian', snr=[5, 2],
                            print_progress=False, max_threads=2)
        files = os.listdir('data/noise/')
        self.assertEqual(len(files), 4)
